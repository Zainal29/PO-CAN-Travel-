...
<div class="mx-auto max-w-xl">
    <div class="border border-emerald-200 bg-white p-6 text-center sm:p-8">
        <div class="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-emerald-100 text-2xl font-bold text-emerald-700">✓</div>
        <p class="mt-5 text-sm font-semibold text-emerald-700">Pembayaran Berhasil</p>
        <h1 class="mt-2 text-2xl font-bold tracking-tight text-brand-950">Tiket Anda sudah tersedia</h1>
        <p class="mt-3 text-sm leading-6 text-gray-600">Pembayaran simulasi internal PO CAN Travel telah berhasil diproses.</p>

        <div class="mt-6 space-y-3 border border-slate-200 bg-slate-50 p-5 text-left text-sm">
        ...
        <div class="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-center">
            <a href="{{ route('customer.orders.ticket', $order) }}" class="btn-primary">Lihat E-Ticket</a>
            <a href="{{ route('customer.orders.show', $order) }}" class="btn-secondary">Detail Pesanan</a>
        </div>
    </div>
</div>
@endsection
